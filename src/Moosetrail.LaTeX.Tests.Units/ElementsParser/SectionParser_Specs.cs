...
{
    [TestFixture]
    public class SectionParser_Specs : FormatterParser_Specs
    {
        ...
        public void setChildElement_should_throw_if_child_is_higher_hirarcy(LaTeXElement element)
        {
            ...
            // Then
            var ex = Assert.Throws<LaTeXException>(() => SUT.SetChildElement(new Formatter(FormatterCommand.section), element));
            Assert.AreEqual("A Section can't have a DocumentClass, Document, Chapter or Section as a child", ex.Message);
        }
        ...
        public void parseCode_should_return_formatter_for_subsubsection()
        {
            ...
            // Then
            Assert.AreEqual(FormatterCommand.section, chapter.Type);
        }
